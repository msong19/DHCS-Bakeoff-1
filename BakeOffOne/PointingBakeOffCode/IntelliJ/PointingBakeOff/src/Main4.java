...
{
    // THIS MAIN SNAPS THE CURSOR TO A SQUARE BASED ON A KEY PRESSED
    ...
    List<Integer> firstRowX = new ArrayList<Integer>();
    List<Integer> firstRowY = new ArrayList<Integer>();
    List<Integer> secondRowX = new ArrayList<Integer>();
    List<Integer> secondRowY = new ArrayList<Integer>();
    List<Integer> thirdRowX = new ArrayList<Integer>();
    List<Integer> thirdRowY = new ArrayList<Integer>();
    List<Integer> fourthRowX = new ArrayList<Integer>();
    List<Integer> fourthRowY = new ArrayList<Integer>();
    String[] secondRow = new String[]{"Q", "W", "E", "R"};
    String[] thirdRow = new String[]{"A", "S", "D", "F"};
    String[] fourthRow = new String[]{"Z", "X", "C", "V"};
    ...
    public void drawButton(int i)
    {
        ...
        // adds the text to each rectangle
        textSize(25);
        fill(0,0,0);
        if(i < 4){
            text(i+1, bounds.x + (bounds.width/2), bounds.y + (bounds.height/2));
            firstRowX.add(bounds.x + (bounds.width/2));
            firstRowY.add(bounds.y + (bounds.height/2));
        }
        else if(i > 3 && i <= 7){
            text(secondRow[i-4],bounds.x + (bounds.width/2), bounds.y + (bounds.height/2));
            secondRowX.add(bounds.x + (bounds.width/2));
            secondRowY.add(bounds.y + (bounds.height/2));
        }
        else if(i > 7 && i <= 11){
            text(thirdRow[i-8], bounds.x + (bounds.width/2), bounds.y + (bounds.height/2));
            thirdRowX.add(bounds.x + (bounds.width/2));
            thirdRowY.add(bounds.y + (bounds.height/2));
        }
        else{
            text(fourthRow[i-12], bounds.x + (bounds.width/2), bounds.y + (bounds.height/2));
            fourthRowX.add(bounds.x + (bounds.width/2));
            fourthRowY.add(bounds.y + (bounds.height/2));
        }
    }
